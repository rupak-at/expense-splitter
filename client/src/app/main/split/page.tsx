    ...
    return (
      ...
      <div className="min-h-screen bg-white">
        <div className="container mx-auto py-8 px-4">
          <h1 className="text-3xl font-bold text-emerald-700 mb-8">Expense Tracker</h1>
      ...
      <div className="container mx-auto py-8 px-4">
        ...
        <Tabs defaultValue="payments" className="w-full">
          ...
          <TabsContent value="payments" className="space-y-4">
            <h2 className="text-2xl font-semibold text-emerald-600 mb-4">Who Pays Who</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {data.map((payment: Split, index: number) => (
                <Card key={index} className="border-emerald-200 hover:shadow-md transition-shadow">
                  <CardHeader className="bg-emerald-50 pb-2">
                    <CardTitle className="text-emerald-700 text-lg">Payment #{index + 1}</CardTitle>
                  </CardHeader>
          ...
          <TabsContent value="expenses" className="space-y-4">
            <h2 className="text-2xl font-semibold text-emerald-600 mb-4">Expense Details</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {expense.map((expense) => (
                <Card key={expense._id} className="border-emerald-200 hover:shadow-md transition-shadow">
                  <CardHeader className="bg-emerald-50 pb-2">
                  ...
                  <CardContent className="pt-4">
                    <div className="flex justify-between items-center text-sm">
                      <div className="flex items-center">
                        ...
                        <span className="text-gray-600">{expense.paidBy.userName}</span>
                      </div>
                      <div className="flex items-center">
                        <Calendar className="h-4 w-4 text-emerald-500 mr-2" />
                        <span className="text-gray-600">{new Date(expense.createdAt).toLocaleDateString()}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
